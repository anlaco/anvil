...
//! ```
//!
//! El host no parsea la línea de comandos: la pasa al guest motor
//! (`inherit_args`), que la parsea como hoy. El host sólo:
//!  1. Arranca el ejecutor (thread) que bindea `127.0.0.1:9100` en su sandbox.
//!  2. Espera a que escuche (un `connect` de prueba; el ejecutor lo descarta).
//!  3. Arranca el motor (main) que conecta, corre la secuencia y sale.
//!  4. Propaga el exit del motor. El thread del ejecutor se aborta al acabar
//!     el proceso.
//!
//! Los dos guests talk por gRPC sobre **loopback TCP** restringido
//! (`socket_addr_check → is_loopback`). El sandbox WASM y el aislamiento
//! motor↔ejecutor (dos `Store`s) se preservan.
