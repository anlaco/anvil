...
//! - Escritura: sólo `Locals`. Escribir en `parameters`/`file_globals` es error
//!   de evaluación (la regla "sólo se muta Locals" se hace valer aquí).
...
/// (por `asigna`); `Parameters` y `FileGlobals` inmutables durante un paso.
/// `resultado` es el paso en curso, expuesto a `asigna`/`precondicion` por la
/// ruta `resultado.*`.
...
impl EntornoMotor {
    ...
    /// parte de sus valores iniciales declarados y muta con `asigna`.
...
impl Entorno for EntornoMotor {
    ...
    fn escribe(&mut self, scope: Scope, campo: &str, valor: Value) -> Result<(), ErrorExpr> {
        match scope {
            ...
            // Regla "sólo se muta Locals": escribir en parameters/file_globals
            // es un error de definición, no se ignora en silencio.
