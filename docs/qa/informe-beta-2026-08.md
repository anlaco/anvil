...
> **Reanclado a `main`.** La campaña corrió contra una implementación del
> process model distinta de la que quedó en `main` (había dos, hechas en
> paralelo; ver ADR-0016). Al incorporar el informe se **re-verificaron los
> seis casos contra `main`**: los seis siguen fallando igual, así que los
> defectos no dependían de aquella implementación. Se han reanclado las
> referencias de fichero:línea y la ruta del process model
> (`process_models/sequential.yaml`), y **DIAG-4 lleva una nota**: en `main`
> el reporte sí registra qué secuencia se ejecutó.
...
**Causa raíz.** En `crates/motor/src/bin/anvil.rs:208` el sidecar se inyecta sobre
...
Con `--process-model`, `cargar_programa_con_pm`
(`crates/cargador/src/lib.rs:670`) hace que **la raíz sea el PM** y la secuencia
...
`anvil.rs:202-204` dice que aplicar el sidecar a las subsecuencias externas es
...
**Causa raíz.** `crates/result_sink/src/csv.rs:81` — `fila_paso` recibe
...
**Atención:** el test unitario de `csv.rs:167` **afirma la salida incorrecta**
...
**Causa raíz.** `crates/motor/src/lib.rs:625` — `ent.escribe(Scope::Locals, &a.var, v)`
...
(`crates/cargador/src/lib.rs:897`), donde un `locals.X` no declarado en el padre
...
<a id="diag-1"></a>
**La mejora más rentable del informe.** Hoy `anvil.rs:210` emite
...
> implementación del process model que enlazaba la secuencia del operador por
> una clave canónica, y el reporte mostraba el placeholder literal:
> `sequence call '__anvil_usuario__' → fallo`. La implementación que quedó en
> `main` (ADR-0016) hace que el **cargador reescriba** el placeholder al path
> real antes de ejecutar, así que el reporte ya dice:
>
> ```
> === sequential: fallo ===
...
> en consola y en el `mensaje` del paso en JSON/CSV. Queda lo menor: ese dato
...
entre `set_resultado` y `limpia_resultado` (`crates/motor/src/lib.rs:621` y `:594`)— y
