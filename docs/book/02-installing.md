...
Download it from the [release page](https://github.com/anlaco/anvil/releases/tag/v0.8.0),
check it, and unpack it:

```console
$ curl -sSLO https://github.com/anlaco/anvil/releases/download/v0.8.0/anvil-v0.8.0-x86_64-linux-musl.tar.gz
$ curl -sSLO https://github.com/anlaco/anvil/releases/download/v0.8.0/SHA256SUMS
$ sha256sum --check --ignore-missing SHA256SUMS
anvil-v0.8.0-x86_64-linux-musl.tar.gz: OK
...
```console
$ export PATH="$PWD/anvil-v0.8.0-x86_64-linux-musl:$PATH"
```
...
Your steps will use a small library, `Anvil.Step`. In 0.8.0 it is not yet
published on NuGet, so you take it from the repository, at the tag that matches
your engine:

```console
$ git clone --depth 1 --branch v0.8.0 https://github.com/anlaco/anvil.git anvil
```
...
├── anvil-v0.8.0-x86_64-linux-musl/      the engine
└── Bench/                               your steps
```
...
files, as `anvil-editor-v0.8.0-x86_64-linux.AppImage`, a `.deb` and a Windows
...
```console
$ chmod +x anvil-editor-v0.8.0-x86_64-linux.AppImage
$ ./anvil-editor-v0.8.0-x86_64-linux.AppImage --appimage-extract
...
On the [release page](https://github.com/anlaco/anvil/releases/tag/v0.8.0),
under **Assets**, download two files. Your browser saves them in `Downloads`:

- `anvil-v0.8.0-x86_64-windows.zip` — the engine;
...
PS> Move-Item "$HOME\Downloads\anvil-v0.8.0-x86_64-windows.zip", "$HOME\Downloads\SHA256SUMS" .
PS> (Get-FileHash .\anvil-v0.8.0-x86_64-windows.zip -Algorithm SHA256).Hash
2F2AA5AF9044B2BBBA42E66434361EBC220EF1849E73F927FFC50121307EC96A
PS> Select-String anvil-v0.8.0-x86_64-windows.zip .\SHA256SUMS
C:\Users\you\anvil-book\SHA256SUMS:5:2f2aa5af9044b2bbba42e66434361ebc220ef1849e73f927ffc50121307ec96a  anvil-v0.8.0-x86_64-windows.zip
```
...
```console
PS> Expand-Archive .\anvil-v0.8.0-x86_64-windows.zip -DestinationPath .
```

It creates `anvil-v0.8.0-x86_64-windows\`, with `anvil.exe` and
...
```console
PS> Get-ChildItem -Recurse .\anvil-v0.8.0-x86_64-windows | Unblock-File
```
...
```console
PS> $env:Path = "$PWD\anvil-v0.8.0-x86_64-windows;$env:Path"
PS> anvil --version
anvil 0.8.0
```
...
```console
PS> git clone --depth 1 --branch v0.8.0 https://github.com/anlaco/anvil.git anvil
PS> dotnet --version
...
├── anvil\                               the repository, for the SDK
├── anvil-v0.8.0-x86_64-windows\         the engine
└── Bench\                               your steps
```
...
| `export PATH="$PWD/anvil-v0.8.0-x86_64-linux-musl:$PATH"` | `$env:Path = "$PWD\anvil-v0.8.0-x86_64-windows;$env:Path"` |
| `2>/dev/null` | `2>$null` |
| `echo $?` | `$LASTEXITCODE` |
...
Download `anvil-editor-v0.8.0-x86_64-windows-setup.exe` from the same page and
